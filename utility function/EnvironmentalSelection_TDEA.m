...
%--------------------------------------------------------------------------
    ...
    objs=vec2mat([Population.factorial_costs],3);  %M=3
    %objs=vec2mat([Population.factorial_costs],2);  %M=2
    [FrontNo,MaxFNo] = NDSort(objs,N);
    St = find(FrontNo<=MaxFNo);

    %% Normalization
    temp=Population(St);
    objs=vec2mat([temp.factorial_costs],3); %M=3
    %objs=vec2mat([temp.factorial_costs],2); %M=2
    [PopObj,z,znad] = Normalization(objs,z,znad);
