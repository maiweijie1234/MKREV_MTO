    ...
    for time=1:totaltimes
        ...
        while FES<=maxFES
            ...
            for k=1:K    
                ...
                objs=vec2mat([temp.factorial_costs],3);
                [z,znad] = deal(min(objs),max(objs));
                xbest=findbest3(temp);  %2-objective is set to findbest2
                xxbest=xbest(1,:);   
                cospop=COSTASK3(P, difference(k,:),simkld(k,:), k); 
                [GAN_modle,~] = gan(temp,Dmax);            
                for i=1:length(temp)                                                             
                    ...
                    if rand<pro                                                                         
                       ...
                       uu=processed3(temp,xgdv,xbest,r,i,1);   %2-objective is set to processed2
...
%                 com_temp = com_temp(rank);
            ...
            for i = 1:K                   
                ...
                T_data = vec2mat([temp_pop.factorial_costs],3);  %M=3
